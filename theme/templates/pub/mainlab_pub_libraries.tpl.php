...
if(count($libraries) > 0){ ?>
  ...
  foreach ($libraries as $library){
     ...
     if ($link) {
       $library_name = l($library_name, $link, array('attributes' => array('target' => '_blank')));
     }
     $organism = $library->organism_id->genus . ' ' . $library->organism_id->species;
     $olink = mainlab_tripal_link_record('organism', $library->organism_id->organism_id);
     if ($olink) {
       $organism = l($organism, $olink, array('attributes' => array('target' => '_blank')));
     }
     $rows[] = array(
       $library_name,
       $library->uniquename,
