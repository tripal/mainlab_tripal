...
if(count($projects) > 0){ ?>
  ...
  foreach ($projects as $project){
     ...
     $link = mainlab_tripal_link_record('project', $project->project_id);
     if ($link) {
       $project_name = l($project_name, $link, array('attributes' => array('target' => '_blank')));
     }
     $description =  substr($project->description, 0, 200);
     if (strlen($project->description) > 200) {
       $description .= "... " . l("[more]", $link, array('attributes' => array('target' => '_blank')));
     }
     $rows[] = array(
       $project_name,
       $description
