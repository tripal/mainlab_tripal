...
if ($properties) {
  foreach ( $properties as $prop ) {
    ...
    elseif ($prop->type_id->name == 'germplasm_center') {
      $germplasm_center= $prop->value;
    }
    elseif ($prop->type_id->name == 'secondary_id') {
      $secondary_id= $prop->value;
    }
  }
}
...
$db = "GRIN ID";
$dbxref = "N/A";
if (isset($stock->dbxref_id->db_id->name)) {
  $db = $stock->dbxref_id->db_id->name == 'GRIN' ? 'GRIN ID' : $stock->dbxref_id->db_id->name;
  $urlprefix = $stock->dbxref_id->db_id->urlprefix;
  $acc = $stock->dbxref_id->accession;
  $ver = $stock->dbxref_id->version;
  if ($ver) {
    $dbxref = "<a href=\"$urlprefix$ver\">$acc</a>";
  }
  else {
    $dbxref = $acc;
  }
...
$stock= chado_expand_var($stock,'table','stock_pub', array('return_array' => TRUE));
$pubs = $stock->stock_pub;
$count_pub = count($pubs);
$rows [] = array(array('data' => 'Publication', 'header' => TRUE, 'width' => '20%'), $pubs ? "[<a href=\"?pane=publications\">view all $count_pub</a>]" : "N/A");
